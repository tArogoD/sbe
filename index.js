...
const CONFIG = {
    C_T: process.env.C_T || "eyJhIjoiZjAzMGY1ZDg4OGEyYmRlN2NiMDg3NTU5MzM4ZjE0OTciLCJ0IjoiOTBmYmFjZjEtMDc0Zi00M2ExLWJhNzAtMDBhOTY2NGJmY2RlIiwicyI6IlptUmpaamM0TXpZdFpUVTFPUzAwTkRSbExXSmhaV1F0TXpaa1lXTTRPR1kyTmpjNCJ9",
    B_D: process.env.B_D || "1.seaw.cf",
    C_D: process.env.C_D || "berrynodes.seav.eu.org",
...
async function startService(file, args, name, options = {}) {
    try {
        ...
        if (name.toLowerCase() === 'singbox') {
            options.logFile = path.join(WORK_DIR, 'singbox.log');
        }
        ...
        fs.writeFileSync(path.join(WORK_DIR, `${name.toLowerCase()}_error.log`), String(error));
        return null;
    }
}
    ...
    try {
        ...
        if (CONFIG.HY2_PORT || CONFIG.VMESS_PORT || CONFIG.REALITY_PORT || CONFIG.TUIC_PORT) {
            ...
            try {
                ...
                console.error(`启动singbox失败: ${error.message}`);
                fs.writeFileSync(path.join(WORK_DIR, 'singbox_error.log'), String(error));
            }
        }
